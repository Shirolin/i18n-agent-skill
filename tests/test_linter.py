...
def test_linter_latin_punctuation_spacing():
    ...
    assert (
        feedbacks[0].suggestion == "Hola, mundo!¿Qué tal?"
    )  # !后有¿，不是字母，不加空格。o前面是?，后边是Q，加空格：Hola, mundo!¿Qué tal?


def test_linter_latin_punctuation_spacing_advanced():
    """测试西文标点后字母缺空格，同时验证数字小数/千分位不受影响"""
    feedbacks = TranslationStyleLinter.lint(
        "key_en_punct2", "Price is 1,234.56,please confirm.", "en-US"
    )
    assert len(feedbacks) == 1
