...
from i18n_agent_skill.__main__ import cli_main, get_dead_keys as cli_get_dead_keys
from i18n_agent_skill.models import ProjectStatus
...
def complex_mock_project(tmp_path, monkeypatch):
    ...
    loc_dir.mkdir()
    (loc_dir / "en.json").write_text(json.dumps({"used": "Used", "en_dead": "Dead"}), encoding="utf-8")
    (loc_dir / "zh-CN.json").write_text(json.dumps({"used": "Used", "zh_dead": "Dead"}), encoding="utf-8")
...
async def test_module_import_safety():
    ...
    try:
        import i18n_agent_skill.__main__
        assert True
    except SystemExit:
